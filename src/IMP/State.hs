...
getProc :: State -> String -> Maybe ([String], [String], Stm)
getProc (_, procs, _) name = Map.lookup name procs

-- | Set procedure.
setProc :: State -> String -> ([String], [String]) -> Stm -> State
setProc (vars, procs, flag) name (params, rets) body =
    (vars, Map.insert name (params, rets, body) procs, flag)
